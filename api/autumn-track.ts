...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { featureId = 'messages', value } = req.body || {};
    ...
    const upstream = await fetch(`${apiBase}/v1/track`, {
      method: 'POST',
      headers: {
    ...
    if (!upstream.ok) {
      console.error('Autumn track error:', upstream.status, payload);
      return withCors(res, allowedOrigin).status(upstream.status).json({ error: 'Track Error', message: payload?.message || 'Failed to record Autumn usage', details: payload });
    }

    return withCors(res, allowedOrigin).status(200).json(payload?.data ?? payload);
  } catch (error) {
    console.error('Autumn track API error:', error);
