...
export class AITeamCoordinator {
  ...
  async startDevelopment(instructions: string): Promise<void> {
    try {
      ...
      const setupResult = await this.projectSetupService.setupProject(projectAnalysis.type, {
        instructions,
        codeContent: '',
      });
  ...
  private async analyzeProjectRequirements(instructions: string): Promise<{
    ...
    return {
      ...
      complexity: lowerInstructions.includes('simple')
        ? 'simple'
        : lowerInstructions.includes('complex') || lowerInstructions.includes('advanced')
          ? 'complex'
          : 'standard',
    };
  }
  ...
  private determineProjectType(instructions: string): string {
    ...
    if (
      instructions.includes('html') ||
      instructions.includes('static') ||
      instructions.includes('landing')
    ) {
      return 'html';
    }
    if (
      instructions.includes('api') ||
      instructions.includes('server') ||
      instructions.includes('express')
    ) {
      return 'generic';
    }
