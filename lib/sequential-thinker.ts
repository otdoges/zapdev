...
async function mcpApiCall(endpoint: string, body: any) {
  if (!MCP_SERVER_URL) {
    errorLogger.warning(
      ...
      'MCP_SEQUENTIAL_THINKING_URL is not set. Skipping MCP server call.'
    );
    return null;
  }
  try {
    ...
    if (!response.ok) {
      errorLogger.error(
        ErrorCategory.GENERAL,
        `MCP Server Error (${endpoint}): ${response.status} ${await response.text()}`
      );
      return null;
    }
    return await response.json();
  } catch (error) {
    errorLogger.error(ErrorCategory.GENERAL, `Error calling MCP Server (${endpoint}):`, error);
    return null;
  }
}
...
async function generateInitialThoughts(
  userPrompt: string,
  modelId: string = 'deepseek/deepseek-chat:free'
): Promise<GeneratedThoughtStructure | null> {
  ...
  try {
    ...
    if (
      ...
      typeof parsedResponse.estimated_total_thoughts === 'number'
    ) {
      return parsedResponse as GeneratedThoughtStructure;
    }
    errorLogger.error(ErrorCategory.GENERAL, 'Failed to parse thought structure from LLM:', text);
    return null;
  } catch (error) {
    errorLogger.error(ErrorCategory.GENERAL, 'Error generating initial thoughts with LLM:', error);
    return null;
  }
}
...
export async function streamSequentialThinking({
  ...
  try {
    ...
    const plan = await generateWithModel(reasoningModelId, [
      {
        role: 'system',
        content:
          'You are a senior software architect. Your task is to analyze the following user prompt and create a detailed plan for implementation. Break down the problem into smaller, manageable steps.',
      },
    ...
    const baseCode = await generateWithModel(codingModelId, [
      {
        role: 'system',
        content:
          'You are a senior software engineer. Your task is to write the base code for the following plan. The code should be well-structured and easy to read.',
      },
      {
        role: 'user',
        content: `Based on the following plan, please write the necessary code:\n\n${plan}`,
      },
    ]);
    ...
    const refinedCode = await generateWithModel(refinementModelId, [
      {
        role: 'system',
        content:
          'You are a code refactoring expert. Your task is to review the following code and improve it. Look for potential bugs, performance issues, or areas where the code could be made more elegant.',
      },
    ...
    data.append(
      JSON.stringify({ type: 'stage', content: 'Performing final review and streaming...' })
    );
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are a senior principal engineer. Your task is to perform a final review of the following code and make any necessary improvements to ensure it is production-ready. Focus on clarity, performance, and best practices. Output only the final code.',
        },
        {
          role: 'user',
          content: `Please perform a final review and refinement of the following code:\n\n${refinedCode}`,
        },
      ],
      onFinish() {
        data.close();
    ...
    errorLogger.error(ErrorCategory.GENERAL, 'Error in sequential thinking process:', error);
 ...
 */
export async function getSequentialThinkingSteps(
  userPrompt: string,
  llmModelForInitialThoughts?: string
): Promise<string | null> {
  await clearHistoryInMcp(); // Start with a clean slate

  const initialThoughtStructure = await generateInitialThoughts(
    userPrompt,
    llmModelForInitialThoughts
  );

  if (
    !initialThoughtStructure ||
    !initialThoughtStructure.thoughts ||
    initialThoughtStructure.thoughts.length === 0
  ) {
    errorLogger.info(
      ErrorCategory.GENERAL,
      'No initial thoughts generated. Skipping MCP processing.'
    );
    return null;
  }
