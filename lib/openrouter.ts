...
export async function generateOpenRouterResponse(
  ...
  try {
    ...
    errorLogger.error(ErrorCategory.GENERAL, 'Error generating response from OpenRouter:', error);
    let errorMessage =
      'Sorry, I encountered an error while processing your request with OpenRouter.';
    if (error instanceof Error) {
      if (error.message.includes('NEXT_OPENROUTER_API_KEY')) {
        errorMessage =
          'The OpenRouter API key is not configured correctly. Please check your environment variables.';
      } else if (error.message.includes('quota') || error.message.includes('limit')) {
        errorMessage =
          'Token limit reached. Please try again later or upgrade your OpenRouter plan.';
      } else {
