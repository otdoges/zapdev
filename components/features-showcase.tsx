...
const features = [
  {
    ...
    description: "Our core AI understands nuance, translating abstract feelings into concrete design elements."
  },
  {
    ...
    description: "Export production-ready code for today's most loved frameworks - Svelte, Astro, Next.js, and more."
  },
  {
  ...
  },
  {
    ...
    description: "Upload an image or URL, and ZapDev intelligently recreates its structure and style."
  },
  {
  ...
  },
  {
    ...
    description: "Access a rich library of pre-built, vibe-matched components that are fully customizable."
  },
  {
...
export default function FeaturesShowcase() {
  ...
  return (
    ...
    >
      {/* Background glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80%] h-[50%] bg-[#4F3A75] opacity-5 blur-[120px] rounded-full" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((feature, index) => (
            <motion.div
              ...
              className={`relative bg-[#121215] rounded-xl p-6 border transition-all duration-300 ${
                activeFeature === index 
                  ? 'border-[#4F3A75] shadow-[0_0_30px_rgba(79,58,117,0.2)]' 
                  : 'border-[#1E1E24] hover:border-[#4F3A75]/40'
              }`}
            >
              <div className={`flex items-center mb-4 text-[#EAEAEA] ${
              ...
              {/* Hover effect glow */}
              {activeFeature === index && (
                <div className="absolute inset-0 -z-10 bg-[#4F3A75]/5 rounded-xl blur-sm" />
              )}
            </motion.div>
          ))}
        </div>
