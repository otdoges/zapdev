  ...
  try {
    ...
    if (!isUpdateAvailable && masterResponse.status === 'fulfilled' && masterResponse.value.ok) {
      ...
      try {
        ...
        // If we can't check package.json, still notify about master commits
        const masterCommitDate = new Date(masterData.commit.committer.date);
        // Use build timestamp or remove this fallback if package.json comparison is the primary method
        const currentVersionDate = new Date(process.env.BUILD_TIMESTAMP || Date.now());        
        if (masterCommitDate > currentVersionDate) {
          isUpdateAvailable = true;
