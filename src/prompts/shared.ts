...
- Prioritize speed: Generate concise, focused code with minimal iterations
- Keep responses brief and to the point
- Avoid unnecessary explanations or verbose comments
- Focus on the core functionality first, then refine if needed
- Use efficient algorithms and minimize redundant operations
...
Generate a brief, user-friendly message (1-2 sentences max) explaining what was built, based on the <task_summary>.
Use casual tone. Focus on the outcome, not the process.
Return only plain text, no code or metadata.
Example: "Built a Netflix-style dashboard with video cards and filtering."
`;

export const FRAGMENT_TITLE_PROMPT = `
Generate a 2-3 word title for the code fragment from <task_summary>.
Title case, no punctuation. Examples: "Netflix Clone", "Todo App", "User Dashboard".
