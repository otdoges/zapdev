...
export const recordUsageEvent = mutation({
  ...
  handler: async (ctx: MutationCtx, args: { eventName: string; metadata: any }) => {
...
export const getUserUsageStats = query({
  ...
  handler: async (ctx: QueryCtx, args: { periodStart?: number; periodEnd?: number }) => {
    const identity = await getAuthenticatedUser(ctx);
...
export const markEventsAsIngested = mutation({
  ...
  handler: async (ctx: MutationCtx, args: { eventIds: Id<"usageEvents">[] }) => {
    const identity = await getAuthenticatedUser(ctx);
    ...
    for (const eventId of args.eventIds) {
      ...
      if ((event as any).userId !== identity.subject) {
