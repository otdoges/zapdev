...
export const upsertUser = mutation({
  ...
  handler: async (ctx: MutationCtx, args: { email: string; fullName?: string; avatarUrl?: string; username?: string; bio?: string }) => {
...
export const updateUserPreferences = mutation({
  ...
  handler: async (ctx: MutationCtx, args: { theme: "light" | "dark" | "system"; settings?: any }) => {
    const identity = await getAuthenticatedUser(ctx);
...
export const deleteUserAccount = mutation({
  ...
  handler: async (ctx: MutationCtx, { confirmEmail }: { confirmEmail: string }) => {
    const identity = await getAuthenticatedUser(ctx);
...
export const upsertUserSubscription = mutation({
  ...
  handler: async (ctx: MutationCtx, args: { userId: string; planId: string; status: string; currentPeriodStart: number; currentPeriodEnd: number; cancelAtPeriodEnd?: boolean }) => {
    const now = Date.now();
    ...
    if (existing) {
      await ctx.db.patch(existing._id, {
        ...
        planType,
        status: args.status as 'active' | 'canceled' | 'past_due' | 'incomplete' | 'trialing' | 'none',
        features,
        usageLimits,
...
export const updateUserAvatar = mutation({
  ...
  handler: async (ctx: MutationCtx, { avatarUrl }: { avatarUrl: string }) => {
    const identity = await getAuthenticatedUser(ctx);
