...
export const setApiKey = mutation({
  ...
  handler: async (ctx: MutationCtx, { provider, apiKey }: { provider: string; apiKey: string }) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) {
      throw new Error("Not authenticated");
    }
...
export const getDecryptedApiKey = mutation({
  ...
  handler: async (ctx: MutationCtx, { provider }: { provider: string }) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) {
      throw new Error("Not authenticated");
    }
...
export const toggleApiKey = mutation({
  ...
  handler: async (ctx: MutationCtx, { provider, isActive }: { provider: string; isActive: boolean }) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) {
      throw new Error("Not authenticated");
    }
