...
export const getSecretChat = query({
  ...
  handler: async (ctx: QueryCtx, { chatId }: { chatId: Id<"secretChats"> }) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) {
      throw new Error("Not authenticated");
    }
    ...
    const messages = await ctx.db
      ...
      .withIndex("by_chat_created", (q: any) => q.eq("chatId", chatId))
      .order("asc")
      .collect();
...
export const createSecretChat = mutation({
  ...
  handler: async (ctx: MutationCtx, { title, provider, model }: { title: string; provider: string; model: string }) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) {
      throw new Error("Not authenticated");
    }
...
export const addSecretMessage = mutation({
  ...
  handler: async (ctx: MutationCtx, { chatId, content, role, metadata }: { chatId: Id<"secretChats">; content: string; role: "user" | "assistant" | "system"; metadata?: { model?: string; tokens?: number; cost?: number } }) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) {
      throw new Error("Not authenticated");
    }
...
export const updateSecretChatTitle = mutation({
  ...
  handler: async (ctx: MutationCtx, { chatId, title }: { chatId: Id<"secretChats">; title: string }) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) {
      throw new Error("Not authenticated");
    }
...
export const deleteSecretChat = mutation({
  ...
  handler: async (ctx: MutationCtx, { chatId }: { chatId: Id<"secretChats"> }) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) {
      throw new Error("Not authenticated");
    }
