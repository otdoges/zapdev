...
export const getSecretChat = query({
  ...
  handler: async (ctx: QueryCtx, { chatId }: { chatId: any }) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) {
      throw new Error("Not authenticated");
    }
...
export const addSecretMessage = mutation({
  ...
  handler: async (ctx: MutationCtx, { chatId, content, role, metadata }: { chatId: any; content: string; role: "user" | "assistant" | "system"; metadata?: { model?: string; tokens?: number; cost?: number } }) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) {
      throw new Error("Not authenticated");
    }
...
export const updateSecretChatTitle = mutation({
  ...
  handler: async (ctx: MutationCtx, { chatId, title }: { chatId: any; title: string }) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) {
      throw new Error("Not authenticated");
    }
...
export const deleteSecretChat = mutation({
  ...
  handler: async (ctx: MutationCtx, { chatId }: { chatId: any }) => {
    const userId = await getAuthUserId(ctx);
    if (!userId) {
      throw new Error("Not authenticated");
    }
